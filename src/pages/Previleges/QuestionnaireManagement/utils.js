...
export const downloadOpenEndedQuestions = (data, name) => {
  let finalData = data;
  ...
  if (name === "CloseEndedQuestions") {
    finalData = finalData.map((item) => {
      let Definition = item.Defination;
      delete item.Defination;
      return {
        ...item,
        Definition
      }
  ...
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");
  XLSX.writeFile(workbook, name + "_" + window.moment(new Date()).format("YYYY-MM-DD-hh-mm-ss") + ".xlsx");
};