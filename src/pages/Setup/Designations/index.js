...
export default function Designation({ grades, departments }) {
  ...
  const handleChangeArray = ({ target: { name, value } }, index) => {
    ...
    // If legal entity is changed, filter departments and clear department selection
    if (name === "legalEntityName") {
      const filtered = allDepartments.filter(
        (dept) => dept.legalEntityName === value
      );
      filtered.unshift({ key: "--Select--", value: "" });
      setFilteredDepartments(filtered);
      updatedData[index].departmentName = ""; // Clear department when legal entity changes
    }
  ...
  const handleSave = () => {
    if (
      ...
      designationInfo[0].legalEntityName.length > 0 &&
      designationInfo[0].status.length > 0
    ) {
      const isEditMode = designationInfo[0]._id !== undefined && designationInfo[0]._id !== null;
      
      // Check for duplicate only when designation + function (department) + grade + legal entity all match
      const isDuplicate = data.some(
        (d) =>
          d._id !== designationInfo[0]._id &&
          d.designationName?.toLowerCase() === designationInfo[0].designationName?.toLowerCase() &&
          d.departmentName?.toLowerCase() === designationInfo[0].departmentName?.toLowerCase() &&
          d.gradeName?.toLowerCase() === designationInfo[0].gradeName?.toLowerCase() &&
          d.legalEntityName?.toLowerCase() === designationInfo[0].legalEntityName?.toLowerCase()
      );

      if (isDuplicate) {
        Toast({
          type: "error",
          message: "A designation with the same designation, function, grade and legal entity already exists.",
        });
        return;
      }
      setLoading(true);
      ...
      if (isEditMode) {
        ...
        const updateDesignationData = {
          _id: designationInfo[0]._id,
          departmentName: designationInfo[0].departmentName,
          designationName: designationInfo[0].designationName,
          gradeName: designationInfo[0].gradeName,
          legalEntityName: designationInfo[0].legalEntityName,
          status: designationInfo[0].status,
          companyId:
        ...
        let result = dispatch(
          updateDesignation(designationInfo[0]._id, updateDesignationData)
        );
...
const handleExport = async ({format}) => {
  try {
    ...
    const exportData = data.map((item) => ({
      "ID": item.id || "",
      "_id": item._id || "",
      [t("departments.designationName")]: item.designationName || "",
      "Legal Entity": item.legalEntityName || "",
      "Function Name": item.departmentName || "",
      [t("departments.gradeName")]: item.gradeName || "",
...
const columns = [
  ...
  {
    id: "designationName",
    label: t("departments.designationName"),
    render: (row) => (
      ...
      return (
        <ActionMenu
          ...
          handleEdit={(data) => {
            ...
            // Filter departments based on legal entity
            if (row.legalEntityName) {
              const filtered = allDepartments.filter(
                (dept) => dept.legalEntityName === row.legalEntityName
              );
              filtered.unshift({ key: "--Select--", value: "" });
              setFilteredDepartments(filtered);
            }
...
const menuItemsExportOptions = [
    ...
    try {
      ...
      response.then(({ data, message }) => {
        if (data !== undefined && data.length > 0) {
          ...
          result.unshift({ key: "--Select--", value: "" });
    ...
    try {
      ...
      response.then(({ data, message }) => {
        if (data !== undefined && data.length > 0 && data[0].departments?.length > 0) {
          let result = data[0].departments.map((dept) => ({
            key: dept.departmentName,
            value: dept.departmentName,
            legalEntityName: dept.legalEntityName,
          }));
          result.unshift({ key: "--Select--", value: "" });
  ...
  const handleFileUpload = async ({ data: designations, file, url }) => {
    const existingDesignationNames = data.map((designation) =>
      designation.designationName.toLowerCase()
    );
    const uploadedDesignationNames = designations.map((designation) =>
      designation.designationName.toLowerCase()
    );

    const duplicates = uploadedDesignationNames.filter((name) =>
      existingDesignationNames.includes(name)
    );

    if (duplicates.length > 0) {
      Toast({
        type: "error",
        message: "Duplicate designations exist, modify the sheet and upload",
      });
      return;
    }
      ...
      <div>
        <div>
          <div style={{ paddingInline: "10px", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <Text style={{ fontSize: "14px", fontWeight: "bold", color: "#4E4A14", margin: 0 }} text="Designations" />
            {designationInfo.length === 0 && canEdit() && (
              <Button
                variant="contained"
          ...
          {designationInfo.map((designation, index) => (
            <div key={index} style={{ marginBottom: "30px", padding: "20px", border: "1px solid #e0e0e0", borderRadius: "8px", backgroundColor: "#fafafa" }}>
              <Text
                ...
                text={designation._id !== undefined && designation._id !== null ? "Edit Designation" : "Add Designation"}
              />
<Row className="g-3 my-2 px-2 py-2 sm:my-0 sm:px-0 sm:py-0">
            ...
            <Col  xs={12} lg={3}>
                  <SelectInput
                    ...
                    value={designation.legalEntityName}
                    onChangeText={(e) => handleChangeArray(e, index)}
                    stackLabel={true}
                    readonly={!canEdit()}
                  />
            ...
            <Col  xs={12} lg={3}>
                  <SelectInput
                    ...
                    onChangeText={(e) => handleChangeArray(e, index)}
                    stackLabel={true}
                    disabled={!designation.legalEntityName || !canEdit()}
                    readonly={!canEdit()}
                  />
                  {validator.current.message(
                    "departmentName",
                ...
                <Row className="g-3 my-2 px-2 py-2 sm:my-0 sm:px-0 sm:py-0">
                  <Col xs={12} className="d-flex justify-content-end align-items-center">
                    <Box sx={{ display: "flex", gap: 1 }}>
                      ...
                      >
                        {designation._id !== undefined && designation._id !== null ? "Update" : "Save"}
                      </Button>
                    </Box>
                  </Col>
                </Row>
