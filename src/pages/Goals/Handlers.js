...
import { updateNotification, updateNotificationTask, updateNotificationGoal } from "action/NotificationAct";
import { useQueryClient } from "@tanstack/react-query";
import { deleteObjectives, updateObjective } from "action/GoalsAct";

export function Handlers(data, setUpdateObj, setLoading, dispatch, props, refreshData, checkCelebration, setEditModal, setError, setEditModalTask, setOrderModalShow3, selectedUsers, setSelectedUsers, setRewardPoints) {
  ...
  const handleCallbackEdit = childData => {
    ...
    response.then(({
      ...
      if (success) {
        ...
        let response3 = dispatch(updateNotificationGoal(childData._id, objectiveStatus));
        response3.then(({
          success,
          message
        }) => {
