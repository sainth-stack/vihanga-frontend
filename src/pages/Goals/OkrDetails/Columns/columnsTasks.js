...
export default function columnsTasks(privileges, setUpdateObj, handleEditViewTask, setViewModalTask, setEditModalTask, handleDeleteTasks) {
  return [
    {
      dataField: "title",
      text: "TITLE",
          ...
          <div className="d-flex flex-wrap">
            <div className="dropdown actionDropdown">
              ...
              >
                ...
                {(row.objectiveStatus === "Unlock" || row.objectiveStatus === "Reject" || row.objectiveStatus === "Create" || row.objectiveStatus === "Update") && privileges && privileges.length > 0 && privileges.filter(privilege => privilege.page === "Tasks").length > 0 && privileges.filter(privilege => privilege.page === "Tasks")[0].edit && <button
                  className="btn btn-default dropdown-item text-capitalize text-left justify-content-start"
                  onClick={() => {
                    setUpdateObj(handleEditViewTask(row));
                ...
                {(row.objectiveStatus === "Unlock" || row.objectiveStatus === "Reject" || row.objectiveStatus === "Create" || row.objectiveStatus === "Update") && privileges && privileges.length > 0 && privileges.filter(privilege => privilege.page === "Tasks").length > 0 && privileges.filter(privilege => privilege.page === "Tasks")[0].delete && <button
                  className="btn btn-default dropdown-item text-capitalize text-left justify-content-start"
      ...
      formatter: (cellContent, row) => {
        return (
          <div className="d-flex flex-wrap">
            <div className="dropdown actionDropdown">
              <a
                href={row.feedAttachment && row.feedAttachment.length > 0 ? row.feedAttachment : "!#"}
                target="_blank"
