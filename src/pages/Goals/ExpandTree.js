...
export default function ExpandTree({ option, index, data, handleInputChange, handleInputChangeObjectives }) {
    ...
    <div key={index + "option"}>
      ...
      {showObjectives && <div className="ml-4">
        {data.length > 0 && data.filter(item => item.okrFunction === option.value).length > 0 && data.filter(item => item.okrFunction === option.value).map((okrFunction, index1) => (
          okrFunction.keyResults.map((child)=>{
            return(
              <ExpandTreeChild index1={index1} data={data} option={option} okrFunction={child} objectiveId={child.objectiveID} handleInputChangeObjectives={handleInputChangeObjectives}
          />
            )
          })
        ))}
