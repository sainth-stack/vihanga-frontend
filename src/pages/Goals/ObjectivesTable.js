...
export default function ObjectivesTable(props) {
  ...
  const { checkboxOptions, filterOptions } = Options(displayOptions, onChangeText, displayOptions2, onChangeText2);
    ...
    try {
      ...
      if (user !== null) {
        ...
        if (data !== undefined && data.length > 0) {
          ...
          let result = tableGenerator(filteredData, filteredData.length, filteredData);
          if (!props.hideHeaders) {
            const { totalWeights, totalWeightsPercent, totalQ1, totalQ2, totalQ3, totalQ4 } = totalQuartersData(result, props.companyInfo);
  ...
  }, [objectivesResponse, props.refresh, props.refresh2, props.companyInfo]);
  ...
  const handleAuditHistory = (id) => {
    ...
    response2.then(({ data, message }) => {
      if (data !== undefined && data.length > 0) {
        ...
        console.log("audit data before set ",data);
  ...
  const { handleEdit, handleDelete, handleDeleteKeyResults, handleViewTask, handleEditTask, handleDeleteTasks, handleCallbackEdit, handleBulkDelete, handlecallback } = Handlers(data, setUpdateObj, setLoading, dispatch, props, refreshData, checkCelebration, setEditModal, setError, setEditModalTask, setOrderModalShow3, selectedUsers, setSelectedUsers, setRewardPoints);
  const { columns, columnsChild, columnsChildTasks } = Columns(handleEdit, setEditModal, privileges, refreshData, props, totalWeight, handleDelete, setOrderModalShow4, setMultipleObjectives, setSelectedObjective, handleDeleteKeyResults, handleViewTask, setViewModalTask, handleEditTask, setEditModalTask, handleDeleteTasks, handleOpenPopup, handleAuditHistory);
  return (
    <>
