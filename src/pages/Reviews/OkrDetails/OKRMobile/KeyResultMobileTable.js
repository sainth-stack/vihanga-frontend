...
export default function KeyResultsMobileTable({ data = [], columns = [], privileges, handleDelete,handleAddTask,handleComments,handleEdit, refreshData }) {
    ...
    <div className='mb-5'>
      {data.length > 0 && data.slice(startPage, pageSize).map((row, index) => (
        <React.Fragment key={row._id}>
          <KeyResultMobileItem privileges={privileges}
