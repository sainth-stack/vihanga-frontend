...
import { createComment, deleteComment, getAllCommentsByReferenceId, updateComment } from "action/CommentsAct";
import { LoadingIndicator } from "utilities";

const EditCommentObjectivePopup = (props) => {
  ...
  const SubmitComment = () => {
    try {
      ...
      if (user !== null) {
        let requestBody = {
          employeeId: user._id,//props.employeeReferenceId,
          employeeName: user.name,//props.employeeName,
          comment: comment,
          ...
          {loading ? <LoadingIndicator /> :
            <div>
              ...
              <div className="text-center">
                <Button onClick={isEdit ? SubmitEdit : SubmitComment} text="Comment" className="bg-brown border text-white" />
              </div>
              <div>
...
const Comment = ({ comment, index, employeeId, employeeName, updatedAt, _id, handleDelete, handleEdit }) => {
  return (
    <div key={index}>
      <div className="d-flex justify-content-between align-items-center">
        <p>
          <img src={userProfile} className="userPic" alt="user" /> {employeeName}  <span title={window.moment(updatedAt).format("DD/MM/YYYY hh:mm:ss A")}>({window.moment(updatedAt).fromNow()})</span></p>
        <div className="d-flex">
          <img src={editIcon} alt="edit" className="p-1 cursor-pointer" onClick={() => handleEdit(_id, comment)} />
...
export default EditCommentObjectivePopup;
