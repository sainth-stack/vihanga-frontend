...
const Objectives1Popup = (props) => {
  ...
  const [objectivesData, setObjectivesData] = useState(
    props.selectedObjective
      ? Array.isArray(props.selectedObjective)
        ? props.selectedObjective
        : [props.selectedObjective]
      : [
          {
    ...
    let resultObjectives = objectivesData.map((singleItem) => {
      ...
      if (filteredObjective.length > 0) {
        ...
        let requestBody = {
          ...
          weight:
            singleItem.weight ??
            filteredObjective[0]?.weight ??
            0,
          keyResults: finalKeyResults,
          objId: singleItem.objectiveId,
          isAlignedToCompany: selectedData.isAlignedToCompany || "Yes",
        };
        return requestBody;
      }
    });
  ...
  useEffect(() => {
    if (props.selectedObjective) {
      const newObjectivesData = Array.isArray(props.selectedObjective)
        ? props.selectedObjective
        : [props.selectedObjective];
