...
import { statuses, statusesActive } from "utilities";
...
import { data } from "components/DashboardComponents/StackChart";
import { useTranslation } from "react-i18next";
import { objectives } from "reducer";
import { displayOpts } from "./../../TasksTableView/ObjectivesTable/defaultData";
      ...
      {!isMobile && (
        <div className="input-group col-lg-6 col-xs-12 col-sm-12 p-0 mt-5 nav-item search-bar">
          ...
          <div className="d-flex">
            <input
              type="text"
              className="bg-gray outline-none searchInput text-dark mt-0 fs14"
              style={{ width: "450px" }}
              placeholder={t("objectives.Search_objective")}
              aria-label="Recipient's username"
              aria-describedby="basic-addon2"
              value={searchKey ? searchKey : ref ? transcript : ""}
      ...
      <div className={`${isMobile ? "" : "col-sm-6"}`}>
        <div className={isMobile ? "d-flex" : "row"}>
          ...
          <Col className={isMobile ? "p-0" : "mt-5 col-sm"}>
            <div className="dropdown actionDropdown" ref={forwardedRef}>
              {isMobile ? (
                <img
                  src={moreIcon}
                  alt="add form"
                ...
                <button
                  className="btn bg-green text-white dropdown-hide text-capitalize p-2  circle border fs16 w-100"
                  type="button"
                  id="dropdownMenuButton"
                  data-toggle="dropdown"
                  aria-haspopup="true"
          ...
          <Col className={isMobile ? "p-0 mt-2" : "mt-5 col-sm"}>
            <SelectInputIcon
              ...
              placeholder={t("objectives.Display_options")}
              name="action"
          ...
          <Col className={isMobile ? "p-0 mt-2" : "mt-5 col-sm"}>
            <SelectInputIcon
              ...
              style={{ backgroundImage: "none", textAlign: "center" }}
              placeholder={t("objectives.Status")}
              name="status"
              options={statusesActive}
