  ...
  const handleSave = () => {
    if (props.selectedData && props.selectedData.length > 0) {
      props.selectedData.forEach((singleItem) => {
        ...
        let keyResults = selectedData.keyResults;
        ...
        finalEmployees = finalEmployees
          ...
          .map((item) => {
            ...
            obj.cascadedObjectiveId = selectedData.objId;
            obj.keyResults = keyResults;
            return obj;
          });
        try {
          setLoading(true);
          let response = dispatch(cascadeObjective(finalEmployees));
          response.then(({ data, message, success }) => {
            if (success) {
              let response2 = dispatch(cascadeObjectiveWithKeyResults(data));
              response2.then(({ data, message, success }) => {
                if (success) {
                  setLoading(false);
                  setError("");
    ...
    if (props.selectedData2 && props.selectedData2.length > 0) {
      props.selectedData2.forEach((singleItem) => {
        let selectedData = singleItem;
        let keyResults = selectedData.keyResults;
        let finalEmployees = [...personalInformation];
        let findId = finalEmployees.filter(
