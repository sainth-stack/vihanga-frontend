...
export default function TableView3({ report }) {
  return (
    <div className="mt-5 text-center">
      <table className="table">
        ...
        <tbody>
          {report.report.scoreSelf.length > 0 && report.report.scoreSelf.filter(item => item.averageScore < 2).length > 0 ? report.report.scoreSelf.sort((a, b) => a.averageScore < b.averageScore ? -1 : 1).map((item, index) => (
            <tr className='row1'>
              ...
              <td className="p-2 text-center">{report.report.scoreManager[index].averageScore}</td>
              <td className="p-2 text-center">{!!report.report.scorePeers ? report.report.scorePeers[index].averageScore : 0}</td>

              <td className="p-3 text-center d-flex justify-content-center align-items-center">
                <div className={`${report.report.groupScore[index].averageScore < 2 ? 'red-circle' : 'green-circle'}`}>{report.report.groupScore[index].averageScore}</div>
              </td>
            </tr>
          )) : <h1>No Data Found!</h1>}
        </tbody>
