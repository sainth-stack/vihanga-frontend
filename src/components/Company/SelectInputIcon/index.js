...
export default function SelectInputIcon({ label = "", placeholder = "", options = [], name = "", value = "", onChangeText, style3, style, checkboxOptions, icon, ...rest }) {
  const isMobile = useWindowSize();
  return (
    <div className="dropdown">
      <button className={`btn dropdown-toggle bg-green text-white dropdown-none text-capitalize SelectButton ${isMobile ? "p-0 m-0" : "border p-2"} fs16 w-100`} type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <img src={icon} alt="eye" className='mr-2' style={{ backgroundColor: "white", borderRadius: "50%", padding: "1px" }} />{isMobile ? "" : placeholder}
      </button>
      <div className="dropdown-menu p-2 zIndex1" aria-labelledby="dropdownMenuButton">
